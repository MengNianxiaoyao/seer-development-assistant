import type { DisplayFormat, HeaderField, ParsedPacket, DiffResult, ParamItem, BodySegment } from '@/types'
...
export function getSendPacket(packets: ParsedPacket[]): ParsedPacket | undefined {
...
export function diffIndexSetFromPackets(packets: ParsedPacket[]): Set<number> {
  if (packets.length < 2) return new Set()
