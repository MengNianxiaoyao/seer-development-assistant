...
export function useHexParser() {
  ...
  function validate(
    ...
    for (let i = 1; i < dataInputs.length; i++) {
      const current = parseSinglePacket(
        0,
        dataInputs[i].raw,
        dataInputs[i].label,
      );
      ...
      // Packet length check
      if (
        current.header.packetLength.hex !== baseline.header.packetLength.hex
      ) {
        reasons.push(
          `封包长度不一致：${current.header.packetLength.decimal} ≠ ${baseline.header.packetLength.decimal}`,
        );
      }
      ...
      if (current.header.paramCount.hex !== baseline.header.paramCount.hex) {
        reasons.push(
          `参数数量不一致：${current.header.paramCount.decimal} ≠ ${baseline.header.paramCount.decimal}`,
        );
      }
      ...
      if (declaredCount > 1 && actualCount !== declaredCount) {
        reasons.push(
          `实际参数数量与声明不一致：解析到 ${actualCount} 个，声明 ${declaredCount} 个`,
        );
      }
      ...
      if (current.params.length !== baseline.params.length) {
        reasons.push(
          `解析参数数量不一致：${current.params.length} ≠ ${baseline.params.length}`,
        );
      }
    ...
    if (
      baselineDeclaredCount > 1 &&
      baselineActualCount !== baselineDeclaredCount
    ) {
      errors.unshift({
        label: dataInputs[0].label,
        reasons: [
          `实际参数数量与声明不一致：解析到 ${baselineActualCount} 个，声明 ${baselineDeclaredCount} 个`,
        ],
      });
    }
