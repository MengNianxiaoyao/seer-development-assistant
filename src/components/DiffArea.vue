...
const { diffPackets, diffIndexSet, hasDiffParams } = usePacketData(computed(() => props.result))

const diffIndices = computed(() => {
  return [...diffIndexSet.value].sort((a, b) => a - b)
})

function getParamValue(packet: { params: ParamItem[] }, index: number): ParamItem | undefined {
  return packet.params.find((p) => p.index === index)
}

function formatParamValue(packet: { params: ParamItem[] }, index: number): string {
  ...
  <div class="panel h-full flex flex-col">
    ...
    <div v-else class="flex-1 overflow-x-auto overflow-y-auto">
      <div class="flex gap-2">
        <div v-for="(packet, pIdx) in diffPackets" :key="packet.id" class="card inline-block flex-shrink-0">
          ...
          <div class="space-y-0.5">
            ...
            >
              ...
              <span v-if="hasParam(packet, idx)">{{ formatParamValue(packet, idx) }}</span>
