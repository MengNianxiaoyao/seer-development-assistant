...
function formatValue(hex: string, decimal: number, binary: string, fmt: DisplayFormat): string {
  switch (fmt) {
...
function getDiffParams(packets: AnalysisResult['packets'], diffIndex: number) {
  return packets.map((p, pIdx) => {
    ...
    return {
      id: p.id,
      label: p.label,
      value: param ? formatValue(param.hex, param.decimal, param.binary, props.format) : '-',
      color: pIdx % 2 === 0 ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600',
    }
  })
}
</script>
...
<template>
  <div class="panel h-full flex flex-col">
    <div class="section-title">输出区 - 相异参数区</div>

    <div v-if="!result || result.diffs.length === 0" class="text-gray-400 text-xs text-center py-4 flex-1">
      暂无相异参数
    </div>
    ...
    <div v-else class="flex-1 overflow-y-auto">
      <div class="flex flex-wrap gap-2">
        <div
          ...
          :key="diff.index"
          class="card border-l-4 border-l-red-400"
        >
