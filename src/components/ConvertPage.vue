...
<template>
  <div class="panel h-full flex flex-col">
    ...
    <div class="flex-1 flex flex-col gap-3 overflow-hidden">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <div class="relative">
          <textarea
            ...
            :placeholder="
              convertDirection === 'hexToFormat' ? '请输入发包文本' : '请输入字节集参数，如 {46046,1,6008}'
            "
          />
          <div class="absolute bottom-2 right-2 flex gap-1">
            <span class="px-1.5 py-0.5 bg-gray-100 rounded text-[10px] text-gray-400 font-mono"
              >{{ hexInput.length }} 字符</span
            >
          </div>
        </div>
      </div>
      ...
      <ConvertResult v-if="showResult" :output="finalOutput" />
    </div>
  </div>
</template>
