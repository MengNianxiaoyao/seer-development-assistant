...
function getHighlightClass(packetIdx: number, paramIdx: number): string {
  if (!diffIndexSet.value.has(paramIdx))
...
<template>
  <div class="panel h-full flex flex-col">
    <div class="section-title">输出区 - 所有参数区</div>

    <div v-if="!result || result.packets.length === 0" class="text-gray-400 text-xs text-center py-4 flex-1">
      点击"开始分析"查看结果
    </div>
